...
#define GET_PTR(dev, x) (void *)(dev->data + (x << SECTOR_SHIFT))
...
static uint __device_queue_count = 1;

module_param(__reset_device, uint, S_IRUGO);
module_param(__device_queue_count, uint, S_IRUGO);

MODULE_PARM_DESC(__reset_device, "Reset device");
MODULE_PARM_DESC(__device_queue_count, "Device queue count");
...
static struct workqueue_struct *csl_workqueue;

struct work_info {
	struct semaphore *work_sema;
	struct work_struct work;
	struct csl_device *dev;
	unsigned long idx;
	void *buf;
	unsigned int len;
};
...
static void read_work_fn(struct work_struct *work)
{
	struct work_info *info = container_of(work, struct work_info, work);
	read_sector(info->dev, info->idx, info->buf, info->len);
	down(info->work_sema);
	kfree(info);
}
...
static void write_work_fn(struct work_struct *work)
{
	struct work_info *info = container_of(work, struct work_info, work);
	write_sector(info->dev, info->idx, info->buf, info->len);
	DOWN_WORK_COUNT(info);
	kfree(info);
}
...
static int dev_request_handle(struct request *rq, unsigned int *nr_bytes)
{
	...
	rq_for_each_segment(bvec, rq, iter) {
		...
		struct work_info *info = (struct work_info *)kmalloc(sizeof(struct work_info), GFP_KERNEL);
		info->work_sema = &work_sema;
		info->dev = dev;
		info->idx = idx;
		info->buf = b_buf;
		info->len = b_len;
		...
		if (rq_data_dir(rq) == WRITE) 
			INIT_WORK(&info->work, write_work_fn);
		else 
			INIT_WORK(&info->work, read_work_fn);

		queue_work(csl_workqueue, &info->work);
...
static int __init csl_driver_init(void)
{
	...
	/* allocate workqueue */
	csl_workqueue = create_workqueue("csl_workqueue");
	if (!csl_workqueue) {
		pr_err("%sFailed to create workqueue\n", PROMPT);
		status = -ENOMEM;
		goto dev_workqueue_fail;
	}
	...
	dev->tag_set->nr_hw_queues = __device_queue_count;
...
dev_register_fail:
	destroy_workqueue(csl_workqueue);
